...
var _addStyle=function(dom,style){
 ...
 if(';'!=st.substr(st.length-1)) st+=';';
 st+=style;
 I.css(dom,st);
};
var _removeStyle=function(dom,styleName){
 if(dom.style.removeProperty) dom.style.removeProperty(styleName);
 else if(dom.style.removeAttribute) dom.style.removeAttribute(_toCamelCase(styleName));
};
var _round=function(dom,n){
 n=n+'';
 var dw='px';
 if(!/^[0-9]*$/.test(n.substr(n.length-1))) dw='';
