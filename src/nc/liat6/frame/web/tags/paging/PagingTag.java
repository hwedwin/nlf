...
public class PagingTag extends TagSupport{
	...
	private void doNormalStartTag(){
		...
		try {
			...
			s.append("D.getElementById('" + fid + "')['"
					+ Request.PAGE_NUMBER_VAR + "'].value=''+o;");
			s.append("};");
			...
			s.append("//]]>\r\n");
			s.append("</script>");
			s.append("<form id=\"" + fid + "\" class=\""+PAGE_FORM_VAR+"\" action=\"" + uri
					+ "\" target=\"_self\" method=\"post\">");
			Map<?, ?> p = pr.getParams();
			for(Object ok:p.keySet()){
				String key = (String) ok;
				if(Request.PAGE_NUMBER_VAR.equals(key)) continue;
				if(Request.PAGE_SIZE_VAR.equals(key)) continue;
				if(Request.PAGE_SORT_VAR.equals(key)) continue;
				s.append("<input name=\"" + key + "\" type=\"hidden\" value=\""
						+ (p.get(key) + "") + "\" />");
			}
			s.append("<input name=\"" + Request.PAGE_NUMBER_VAR
					+ "\" type=\"hidden\" value=\"" + pd.getPageNumber()
					+ "\" />");
			s.append("<input name=\"" + Request.PAGE_PARAM_VAR
					+ "\" type=\"hidden\" value=\"" + sPageRequest + "\" />");
			s.append("<input name=\"" + Request.PAGE_SIZE_VAR
					+ "\" type=\"hidden\" value=\"" + pd.getPageSize()
					+ "\" />");
