...
public abstract class AopManager extends AbstractManager {
	...
	private boolean checkMethod(String methodName, String klassName, String method) {
		...
		case 0:
			...
			// [] [test*] [test]
		case 1:
			if (methodName.startsWith(mn[0]))	return true;
			// [*Test] [test*A]
		case 2:
			if (methodName.startsWith(mn[0]) && methodName.endsWith(mn[1])) return true;
		}
		return false;
	}
	...
	public boolean check(Object o, Method m) {
		...
		//处理代理类
		if(klassName.indexOf("$")>-1)	klassName = klassName.substring(0,klassName.indexOf("$"));
		if ("finalize".equals(methodName)) return false;
